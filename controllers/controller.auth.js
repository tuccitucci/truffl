...
module.exports = {
    render: (req, res) => {
        if (req.session.uid) {
            ...
            res.sendFile('auth', req.session); // render the authenticaiton page (register/login)
        }
    },
    logout: (req, res) => {
            ...
            if (!user) {
                ...
                bcrypt.compare(req.body.password, user.password, (bcryptErr, matched) => {
                    if (bcryptErr) {
                        ...
                        res.send(req.session); // send a success message
                    }
                });
            }
        });
    ...
    getMe: (req, res) => {
      User.findOne({
        ...
        if (err) {
          ...
          console.log('Good to go getting User info: ', user);
          res.send(user);
        }
      })
    },
